...
def saveEvaluation(eval_return,eval_num_of_step,num):
	eval_num_of_step = np.array(eval_num_of_step)
	eval_return = np.array(eval_return)
	np.save('eval_goal_c/eval_return_agent_' + str(num),eval_return)
	np.save('eval_goal_c/eval_num_steps_agent_' + str(num),eval_num_of_step)
...
for i in range(len(goal_states)):
	for j in range(num_of_agents):
		...
		RLGlue.RL_agent_message("set-epsilon 0.5")
		returns = []
		num_steps = []
		eval_return = []
		eval_num_of_step = []
		for episode in range(num_of_episode):
			...
			if episode == num_of_episode - 1:
				RLGlue.RL_agent_message("save_policy result_goal_C_agent_" + str(j+1) + ".dat") # Save the value function for the agent
			if (episode % 25 == 0):
				RLGlue.RL_agent_message("decay-epsilon")
				evaluateAgent(eval_return,eval_num_of_step)
		returns = np.array(returns)
		num_steps = np.array(num_steps)		
		np.save('returns_goal_c/return_' + 'agent_' + str(j) + '_goal_' + goal_states[i],returns)
		np.save('num_steps_goal_c/num_step_' + 'agent_' + str(j) + '_goal_' + goal_states[i],num_steps)
		saveEvaluation(eval_return,eval_num_of_step,j)