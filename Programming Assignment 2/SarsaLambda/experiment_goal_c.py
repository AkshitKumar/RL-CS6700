		...
		for j in range(num_of_agents):
			...
			for episode in range(num_of_episode):
				...
				if episode == num_of_episode - 1:
					RLGlue.RL_agent_message("save_policy result_goal_C_agent_" + str(j+1) + "_lambda_" + str(lambda_val) + ".dat") # Save the value function for the agent
			returns = np.array(returns)
			num_steps = np.array(num_steps)		
			np.save('returns_goal_c/return_' + 'agent_' + str(j) + '_lambda_' + str(lambda_val) + '_goal_' + goal_states[i],returns)
			np.save('num_steps_goal_c/num_step_' + 'agent_' + str(j) + '_lambda_' + str(lambda_val) + '_goal_' + goal_states[i],num_steps)
