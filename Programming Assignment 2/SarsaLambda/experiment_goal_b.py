...
def saveEvaluation(eval_return,eval_num_of_step,num,lambda_val):
	...
	np.save('eval_goal_b/eval_return_agent_' + str(num) + '_lambda_' + str(lambda_val),eval_return)
	np.save('eval_goal_b/eval_num_steps_agent_' + str(num) + '_lambda_' + str(lambda_val),eval_num_of_step)
		...
		for j in range(num_of_agents):
			...
			for episode in range(num_of_episode):
				...
				if episode == num_of_episode - 1:
					RLGlue.RL_agent_message("save_policy result_goal_B_agent_" + str(j+1) + "_lambda_" + str(lambda_val) + ".dat") # Save the value function for the agent
			returns = np.array(returns)
			num_steps = np.array(num_steps)		
			np.save('returns_goal_b/return_' + 'agent_' + str(j) + '_lambda_' + str(lambda_val) + '_goal_' + goal_states[i],returns)
			np.save('num_steps_goal_b/num_step_' + 'agent_' + str(j) + '_lambda_' + str(lambda_val) + '_goal_' + goal_states[i],num_steps)
